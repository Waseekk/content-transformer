...
from shared.config.settings import SCRAPER_CONFIG, RAW_DATA_DIR, SITES_CONFIG_PATH
from shared.utils.logger import get_scraper_logger
...
class MultiSiteScraper:
    ...
    def __init__(self, status_callback=None, enabled_sites: Optional[List[str]] = None):
        """
        Initialize multi-site scraper

        Args:
            status_callback: Optional callback function(status) for status updates
            enabled_sites: Optional list of site names to scrape (filters sites_config)
        """
        ...
        all_sites = self.load_sites_config()

        # Filter sites if enabled_sites is provided
        if enabled_sites:
            self.sites_config = [site for site in all_sites if site['name'] in enabled_sites]
            logger.info(f"Filtering to {len(self.sites_config)} enabled sites from {len(all_sites)} total")
        else:
            self.sites_config = all_sites
            logger.info(f"Using all {len(self.sites_config)} sites (no filter applied)")
